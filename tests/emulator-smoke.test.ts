...
vi.mock('@/media/floppy/floppy-sound.ts', () => ({ FloppySound: function() { return { reset: vi.fn(), destroy: vi.fn() }; } }));
vi.mock('@/machines/spectrum/ula.ts', () => ({ PALETTES: { measured: [] }, SCREEN_WIDTH: 320, SCREEN_HEIGHT: 240 }));
...
vi.mock('@/managers/rom-manager.ts', () => ({ ROMManager: class { restoreROM = vi.fn(async () => null); fetchDefaultROM = vi.fn(async () => ({ data: new Uint8Array(16384), label: '48k' })); persistROM = vi.fn(); }, resolveRomSource: (source: string) => source }));
...
vi.mock('@/machines/spectrum/peripherals/multiface.ts', () => ({ variantForModel: vi.fn(() => 'mf128'), variantLabel: vi.fn(() => 'Multiface 128'), romFilename: vi.fn(() => 'mf128.rom') }));
vi.mock('@/machines/spectrum/peripherals/joysticks.ts', () => ({ KEMPSTON_BITS: {}, CURSOR_KEYS: {}, SINCLAIR1_KEYS: {}, SINCLAIR2_KEYS: {}, resetJoystickKeyState: vi.fn(), joyPressForType: vi.fn() }));
vi.mock('@/machines/spectrum/snapshots/szx.ts', () => ({ saveSZX: vi.fn(async () => new Uint8Array()), loadSZX: vi.fn(async () => ({ is128K: false, borderColor: 0, port7FFD: 0, port1FFD: 0 })) }));
vi.mock('@/machines/spectrum/snapshots/z80format.ts', () => ({ saveZ80: vi.fn(() => new Uint8Array()) }));
vi.mock('@/media/tape/tzx.ts', () => ({ parseTZX: vi.fn(() => []) }));
vi.mock('@/media/floppy/dsk.ts', () => ({ parseDSK: vi.fn(() => ({ tracks: [] })), serializeDSK: vi.fn(() => new Uint8Array()) }));
