...
import { SymbolTable } from './debug/symbols.ts';
...
function parseAddr(s: string): number {
  ...
  if (/^[A-Za-z_]/.test(raw)) {
    ...
    // No match — fall through. Most likely a typo, but parseInt('0xff', 16)
    // semantics would mask that, so we surface NaN instead by returning a
    // hex parse, which will produce NaN for non-hex input.
  }
...
// ── Symbol table ─────────────────────────────────────────────────────────
//
// Symbols loaded here are resolved transparently by parseAddr(), so any
// tool that takes an address (read_memory, breakpoint, disassemble, ...)
// will accept symbol names too.
