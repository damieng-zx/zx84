 ...
 *
 * The WD1772 (a Western Digital FD179x-family controller) is far simpler than
 * the uPD765A on the +3: four directly-addressed registers and a synchronous
 * command model with no command/result phases.
 *
 *   Status / Command   (read / write)   +D port 0xE3
 *   Track              (read / write)   +D port 0xEB
 *   Sector             (read / write)   +D port 0xF3
 *   Data               (read / write)   +D port 0xFB
 *
 * The drive and side are selected by the +D control register (port 0xEF),
 * handled by the MgtPlusD peripheral which calls selectDrive() / setSide().
 *
 ...
 * register — exactly the polling loop G+DOS uses (poll status, read/write data,
 * repeat while BUSY). Disk images are the shared DskImage structure (see
 * plus3/dsk.ts) so geometry, sector lookup and write-back match the +3 path and
 * the existing disk UI / floppy-sound code can be reused unchanged.
 *
...
const ST_MOTORON   = 0x80; // WD1772: motor on (no READY line on the 1770/1772)
...
// INDEX pulse emulation. G+DOS confirms a disk is spinning by watching status
// bit 1 (INDEX, in Type I status) transition 0→1. We synthesise that pulse from
// a per-status-read counter so the edge always appears: bit 1 reads 1 for the
// first INDEX_WIDTH of every INDEX_PERIOD status reads, else 0.
...
export class WD1772 {
  ...
  // ── Drive / head state ────────────────────────────────────────────────
  /** Selected drive (0 or 1) — set by the +D control register. */
  currentDrive = 0;
  /** Selected side (0 or 1) — set by the +D control register. */
  side = 0;
  motorOn = false;
  ...
  /** Status register (port 0xE3 read). In Type I status, synthesise the INDEX
   *  pulse (bit 1) when a disk is present and spinning so G+DOS's index-edge
   *  detector sees the 0→1 transition it needs. */
  readStatus(): number {
  ...
  /** Data register (port 0xFB read) — streams sector bytes during a read. */
  readData(): number {
  ...
  /** Data register (port 0xFB write) — sector bytes during write, else the
   *  SEEK target track or a register load. */
  ...
  // ── Command register (port 0xE3 write) ────────────────────────────────
  ...
  private base(): number { return this.motorOn ? ST_MOTORON : 0; }
  ...
  private readTrackCmd(): void {
    // Rarely used by G+DOS; serve the concatenated sector data of the track.
    const track = this.locateTrack();
    if (!track) { this.statusReg = this.base() | ST_RNF; return; }
    ...
    switch (this.fmtState) {
      ...
      case 'data':
        ...
        if (--this.fmtDataLeft <= 0) {
          ...
          this.fmtPending = null;
          this.fmtState = 'idle';
          // A full G+DOS track is 10 sectors; finalise as soon as we have them.
          if (this.fmtSectors.length >= 10) this.finishFormat();
        }
        break;
    }
  }
