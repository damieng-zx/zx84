...
/** The key-matrix source the ULA reads on a port-0xFE `IN`. A real ULA drives
 *  the address lines and reads back the selected half-rows — it doesn't know
 *  about a "keyboard object". `SpectrumKeyboard` satisfies this structurally. */
export interface KeyMatrixSource {
...
export class ULA {
  ...
  /** Reference to the key matrix for port reads */
  keyboard: KeyMatrixSource;

  constructor(keyboard: KeyMatrixSource) {
