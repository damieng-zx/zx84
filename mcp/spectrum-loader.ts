/**
 * Spectrum bench loading — the MCP's Spectrum-specific media path, split out
 * of the generic loader. Everything here operates on an already-narrowed
 * `Spectrum` (callers go through concrete.ts's activeSpectrum()).
 *
 * This deliberately does MORE than the machine's own MediaService: it
 * auto-enables the +D / Interface 1 / Beta Disk ROMs for peripheral media
 * (the service refuses instead), auto-detects SZX models, and arms the ROM
 * auto-boot traps used by the library loader. The trade-off is acceptable
...
import { loadZ80 } from '../src/machines/spectrum/snapshots/z80format.ts';
import { loadSZX } from '../src/machines/spectrum/snapshots/szx.ts';
import { parseFloppyImage } from '../src/media/floppy/hfe.ts';
import { parseMgt, mgtExtFromName } from '../src/media/floppy/mgt-image.ts';
import { parseTrd } from '../src/media/floppy/trd-image.ts';
